...
public:
   ...
   //--- IMTConSymbol::EnMarginCheck
   virtual UINT      MarginCheckMode(void) const=0;
   virtual MTAPIRES  MarginCheckMode(const UINT mode)=0;
   virtual UINT      MarginCheckModeDefault(void) const=0;
   //--- initial margin
...
public:
   ...
   enum EnPermissionsFlags
     {
      PERMISSION_NONE             =0x0,  // default
      PERMISSION_CERT_CONFIRM     =0x1,  // certificate confirmation neccessary
      PERMISSION_ENABLE_CONNECTION=0x2,  // clients connections allowed
      PERMISSION_RESET_PASSWORD   =0x4,  // reset password after first logon
      PERMISSION_FORCED_OTP_USAGE =0x8,  // принудительное OTP
      //--- enumeration borders
      PERMISSION_ALL         =PERMISSION_CERT_CONFIRM|PERMISSION_ENABLE_CONNECTION|PERMISSION_RESET_PASSWORD|PERMISSION_FORCED_OTP_USAGE
     };
   //--- authorization mode
   enum EnAuthMode
     {
   ...
   enum EnAuthOTPMode
     {
      ...
      AUTH_OTP_LAST               =AUTH_OTP_TOTP_SHA256
     };
   ...
   enum EnMarginMode
     {
      MARGIN_MODE_RETAIL          =0,  // Retail FX, Retail CFD, Retail Fututes
      MARGIN_MODE_EXCHANGE_DISCOUNT=1, // Exchange, mrgin discount rates based 
      //--- enumeration borders
      MARGIN_MODE_FIRST            =MARGIN_MODE_RETAIL,
      MARGIN_MODE_LAST             =MARGIN_MODE_EXCHANGE_DISCOUNT
     };
   //--- trade rights flags
   enum EnTradeFlags
     {
