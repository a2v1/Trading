...
public:
   ...
   void              Replace(wchar_t *findstr,wchar_t *repstr);
...
//| Replace substring                                                |
//+------------------------------------------------------------------+
inline void CMTStr::Replace(wchar_t *findstr,wchar_t *repstr)
  {
//--- check
   if(findstr && repstr && m_str)
     {
      wchar_t *cp;
      size_t   findstr_len=wcslen(findstr);
      //--- find
      while((cp=wcsstr(m_str,findstr))!=NULL)
        {
         size_t   newsize=m_str_max+wcslen(repstr);
         wchar_t *newstr=new(std::nothrow) wchar_t[newsize+1];
         //--- check
         if(newstr==NULL) break;
         //--- collect new buffer
         *cp=0;
         int len=_snwprintf_s(newstr,newsize,_TRUNCATE,L"%s%s%s",cp,repstr,cp+findstr_len);
         if(len>=0)
           {
            m_str_len=UINT(len);
            //--- copy buffer
