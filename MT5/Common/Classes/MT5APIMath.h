...
public:
   //--- common functions
   static double __fastcall DecPow(INT digits);
   //--- price functions
...
//| Decimal power                                                    |
//+------------------------------------------------------------------+
inline double __fastcall SMTMath::DecPow(INT digits)
  {
//--- check sign
   if(digits>0 && digits<9)  return(s_decimal[digits]);
   else
      if(digits<0 && digits>-9) return(s_fdecimal[-digits]);
//--- use standard pow
...
inline double SMTMath::PriceNormalize(const double price,UINT digits)
  {
   double dbl_integer;
//--- check digits
   if(digits>MTAPI_PRICE_DIGITS_MAX) digits=MTAPI_PRICE_DIGITS_MAX;
//--- calculate dividers
...
inline INT64 SMTMath::PriceToInt(const double price,const UINT digits)
  {
//--- check
   if(digits>MTAPI_PRICE_DIGITS_MAX || price==0.0 || price>MTAPI_PRICE_MAX) return(0);
//--- calculate
