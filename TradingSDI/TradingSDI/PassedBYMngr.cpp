
#include "stdafx.h"
#include "resource.h"
#include "PassedBYMngr.h"
#include <afxdisp.h>
#include <time.h>
#include <locale.h>
#include "MainFrm.h"
#include "document.h"
#include <iostream>
#include "Login.h"
#include "PassedBYMngrTable.h"

using namespace std;
using namespace rapidjson;

#ifdef _DEBUG
#define new DEBUG_NEW
#undef THIS_FILE
static char THIS_FILE[] = __FILE__;
#endif

BEGIN_MESSAGE_MAP(CPassedBYMngr,CUGCtrl)	
	ON_WM_SIZE()
	ON_WM_TIMER()		
END_MESSAGE_MAP()

int CPassedBYMngr::insertFilterFlag=0;
CPassedBYMngr::st_Dealing_Array CPassedBYMngr::m_st_Dealing_Array;	
CPassedBYMngr::st_Dealing_Array CPassedBYMngr::m_st_Dealing_Grid_array;
CMutex CPassedBYMngr::dealing_mutex;

CString CPassedBYMngr::col0_val=L"";
CString CPassedBYMngr::col1_val=L"";
CString CPassedBYMngr::col2_val=L"";
CString CPassedBYMngr::col3_val=L"";
CString CPassedBYMngr::col4_val=L"";
CString CPassedBYMngr::col5_val=L"";
CString CPassedBYMngr::col6_val=L"";
CString CPassedBYMngr::col7_val=L"";
CString CPassedBYMngr::col8_val=L"";
CString CPassedBYMngr::col9_val=L"";
CString CPassedBYMngr::col10_val=L"";
CString CPassedBYMngr::col11_val=L"";
CString CPassedBYMngr::col12_val=L"";
CString CPassedBYMngr::col13_val=L"";
CString CPassedBYMngr::col14_val=L"";
int CPassedBYMngr::Data_Update=0;

UINT PassedByMnger(void *pParam);
CPassedBYMngr::CPassedBYMngr(void)
{
	UGXPThemes::UseThemes(false);
	m_pThreads=NULL;
	col_click=0;
	a_d=0;
}


CPassedBYMngr::~CPassedBYMngr(void)
{
	//delete m_pThread;
	UGXPThemes::CleanUp();
}

int CPassedBYMngr::OnCellTypeNotify(long ID,int col,long row,long msg,long param)
{

	if(ID == UGCT_DROPLIST)
	{
		return OnDropList(ID, col, row, msg, param);
	}

	return 0;
}

void CPassedBYMngr::data_ThreadStart()
{
	CPassedBYMngr::Data_Update=1;
	m_pThreads=AfxBeginThread(PassedByMnger, this);		
}

void CPassedBYMngr::OnSetup()
{
	try
	{					
		int	nIndex = 0;
		CUGCell cell;
		GetGridDefault(&cell);
		SetGridDefault(&cell);
		EnableExcelBorders(FALSE);
		SetHighlightRow(TRUE, FALSE);
		SetDoubleBufferMode(TRUE);
		SetUserSizingMode( TRUE );			
		// Create and set default font
		AddFont(L"Arial", -12, FW_NORMAL);
		AddFont(L"Arial", -12, FW_BOLD);
		SetDefFont(0);	
		CDC * pDC;	
		int iHeight;
		int i, j;	
		pDC = GetDC( );	
		iHeight = -MulDiv( 8, GetDeviceCaps( pDC->m_hDC, LOGPIXELSY), 72 );
		m_font.CreateFont( iHeight, 0, 0, 0, 400, 0, 0, 0, 0, 0, 0, 0, 0, _T( "MS Sans Serif" ) );	
		ReleaseDC( pDC );
		SetDefFont( &m_font );		
		GetGridDefault( &cell );
		cell.SetAlignment( UG_ALIGNCENTER | UG_ALIGNVCENTER );
		SetGridDefault( &cell );
		SetNumberCols( 7);		
		m_iArrowIndex = AddCellType( &m_sortArrow );
		SetCurrentCellMode( 2 );
		UseHints( TRUE );
		SetSH_Width( 0 );	
		QuickSetCellType( 0, -1, m_iArrowIndex );
		SortBy( 0, UG_SORT_ASCENDING );
		m_iSortCol = 0;
		m_bSortedAscending = TRUE;		
		InitMenu();
		SetTimer(0, 100, NULL);

		//m_pThreads=AfxBeginThread(PassedByMnger, this);	
	}
	catch(_com_error & ce)
	{
		AfxMessageBox(ce.Description()+L"   writeData");			
	}
}

void CPassedBYMngr::InitMenu()
{
	CMenu submenu;
	CMenu * menu = GetPopupMenu();

	EnableMenu(TRUE);

	menu->AppendMenuW(MF_STRING|MF_UNCHECKED,2001,_T("Filter"));


	submenu.CreatePopupMenu();
}

void CPassedBYMngr::OnSheetSetup(int sheetNumber)
{
	int	nRow = 0, nCol = 0;	
	CUGCell cell;	
	SetGridDefault(&cell);
	EnableExcelBorders(TRUE);
	SetHighlightRow(TRUE, FALSE);
	SetDoubleBufferMode(TRUE);
	SetDefFont(0);
	SetSH_Width(0);		

	SetNumberCols(15);

	QuickSetText(0,-1,L"SL No");
	SetColWidth(0,50);
	QuickSetText(1,-1,L"Passed By Manager");
	SetColWidth(1,140);
	QuickSetText(2,-1,L"Login");
	SetColWidth(2,70);
	QuickSetText(3,-1,L"Order#");	
	SetColWidth(3,70);			
	QuickSetText(4,-1,L"Order_In_Time");
	SetColWidth(4,180);			
	QuickSetText(5,-1,L"Deal#");
	SetColWidth(5,70);
	QuickSetText(6,-1,L"Symbol");	
	SetColWidth(6,100);
	QuickSetText(7,-1,L"Type");
	SetColWidth(7,70);
	QuickSetText(8,-1,L"Volume");
	SetColWidth(8,80);			
	QuickSetText(9,-1,L"Price");
	SetColWidth(9,70);
	QuickSetText(10,-1,L"Comment");
	SetColWidth(10,80);	
	QuickSetText(11,-1,L"Status");
	SetColWidth(11,60);
	QuickSetText(12,-1,L"Type");
	SetColWidth(12,60);
	QuickSetText(13,-1,L"SubType");
	SetColWidth(13,60);
	QuickSetText(14,-1,L"PassedTime");
	SetColWidth(14,160);

	// Header font
	for(int i = 0; i < GetNumberCols(); i++)
	{
		QuickSetFont(i, -1, 1);
	}	
}

void CPassedBYMngr::OnTimer(UINT nIDEvent)
{		

	//m_st_Dealing_Grid_array.Assign(m_st_Dealing_Array);
	CPassedBYMngr::dealing_mutex.Lock();	

	int noof_rowsInStruc=CPassedBYMngr::m_st_Dealing_Array.Total();	

	if (noof_rowsInStruc>0)
	{
		m_st_Dealing_Grid_array.Clear();
	}

	for(int fcount=0;fcount<noof_rowsInStruc;fcount++)
	{
		if(CPassedBYMngr::insertFilterFlag==1)
		{
			CString hold_filtrVal=L"";
			CPassedBYMngr::st_Dealing m_st_Netposition={};
			m_st_Netposition=CPassedBYMngr::m_st_Dealing_Array[fcount];

			int flag=0;				
			CString col_row_val[15];

			col_row_val[1]=m_st_Netposition.Section  ;
			if (CPassedBYMngr::col1_val.Trim().GetLength()>0)
			{
				col_row_val[1]=col_row_val[1].Mid(0,CPassedBYMngr::col1_val.Trim().GetLength());
			}
			col_row_val[2]=m_st_Netposition.Login   ;
			if (CPassedBYMngr::col2_val.Trim().GetLength()>0)
			{
				col_row_val[2]=col_row_val[2].Mid(0,CPassedBYMngr::col2_val.Trim().GetLength());
			}



			hold_filtrVal.Format(L"%.0f",m_st_Netposition.Order  );
			col_row_val[3]=hold_filtrVal  ;
			if (CPassedBYMngr::col3_val.Trim().GetLength()>0)
			{
				col_row_val[3]=col_row_val[3].Mid(0,CPassedBYMngr::col3_val.Trim().GetLength());
			}



			col_row_val[4]=m_st_Netposition.Order_In_Time; 		
			if (CPassedBYMngr::col4_val.Trim().GetLength()>0)
			{
				col_row_val[4]=col_row_val[4].Mid(0,CPassedBYMngr::col4_val.Trim().GetLength());
			}




			hold_filtrVal.Format(L"%d",m_st_Netposition.Deal );
			col_row_val[5]=hold_filtrVal;
			if (CPassedBYMngr::col5_val.Trim().GetLength()>0)
			{
				col_row_val[5]=col_row_val[5].Mid(0,CPassedBYMngr::col5_val.Trim().GetLength());
			}



			col_row_val[6]=m_st_Netposition.Symbol;		
			if (CPassedBYMngr::col6_val.Trim().GetLength()>0)
			{
				col_row_val[6]=col_row_val[6].Mid(0,CPassedBYMngr::col6_val.Trim().GetLength());
			}


			col_row_val[7]=m_st_Netposition.Type1 ;
			if (CPassedBYMngr::col7_val.Trim().GetLength()>0)
			{
				col_row_val[7]=col_row_val[7].Mid(0,CPassedBYMngr::col7_val.Trim().GetLength());
			}


			hold_filtrVal.Format(L"%.2f",m_st_Netposition.Volume );
			col_row_val[8]=hold_filtrVal  ;
			if (CPassedBYMngr::col8_val.Trim().GetLength()>0)
			{
				col_row_val[8]=col_row_val[8].Mid(0,CPassedBYMngr::col8_val.Trim().GetLength());
			}


			hold_filtrVal.Format(L"%.4f",m_st_Netposition.Price );
			col_row_val[9]=hold_filtrVal;
			if (CPassedBYMngr::col9_val.Trim().GetLength()>0)
			{
				col_row_val[9]=col_row_val[9].Mid(0,CPassedBYMngr::col9_val.Trim().GetLength());
			}			


			col_row_val[10]=m_st_Netposition.Comment ;
			if (CPassedBYMngr::col10_val.Trim().GetLength()>0)
			{
				col_row_val[10]=col_row_val[10].Mid(0,CPassedBYMngr::col10_val.Trim().GetLength());
			}

			col_row_val[11]=m_st_Netposition.Status ;
			if (CPassedBYMngr::col11_val.Trim().GetLength()>0)
			{
				col_row_val[11]=col_row_val[11].Mid(0,CPassedBYMngr::col11_val.Trim().GetLength());
			}


			col_row_val[12]=m_st_Netposition.Type ;
			if (CPassedBYMngr::col12_val.Trim().GetLength()>0)
			{
				col_row_val[12]=col_row_val[12].Mid(0,CPassedBYMngr::col12_val.Trim().GetLength());
			}

			col_row_val[13]=m_st_Netposition.SubType ;
			if (CPassedBYMngr::col13_val.Trim().GetLength()>0)
			{
				col_row_val[13]=col_row_val[13].Mid(0,CPassedBYMngr::col13_val.Trim().GetLength());
			}


			col_row_val[14]=m_st_Netposition.PassedTime ;
			if (CPassedBYMngr::col14_val.Trim().GetLength()>0)
			{
				col_row_val[14]=col_row_val[14].Mid(0,CPassedBYMngr::col14_val.Trim().GetLength());
			}

			if((CPassedBYMngr::col1_val.Trim()==col_row_val[1].Trim() || CPassedBYMngr::col1_val.Trim()==L"ALL"||CPassedBYMngr::col1_val.Trim()==L"") && (CPassedBYMngr::col2_val.Trim()==col_row_val[2].Trim() || CPassedBYMngr::col2_val.Trim()==L"ALL"||CPassedBYMngr::col2_val.Trim()==L"")  && (CPassedBYMngr::col3_val.Trim()==col_row_val[3].Trim() || CPassedBYMngr::col3_val.Trim()==L"ALL"||CPassedBYMngr::col3_val.Trim()==L"")  && (CPassedBYMngr::col4_val.Trim()==col_row_val[4].Trim() ||CPassedBYMngr::col4_val.Trim()==L"ALL"||CPassedBYMngr::col4_val.Trim()==L"")   && (CPassedBYMngr::col5_val.Trim()==col_row_val[5].Trim() || CPassedBYMngr::col5_val.Trim()==L"ALL"||CPassedBYMngr::col5_val.Trim()==L"")   && (CPassedBYMngr::col6_val.Trim()==col_row_val[6].Trim() || CPassedBYMngr::col6_val.Trim()==L"ALL"||CPassedBYMngr::col6_val.Trim()==L"")   && (CPassedBYMngr::col7_val.Trim()==col_row_val[7].Trim() || CPassedBYMngr::col7_val.Trim()==L"ALL"||CPassedBYMngr::col7_val.Trim()==L"")  && (CPassedBYMngr::col8_val.Trim()==col_row_val[8].Trim() ||CPassedBYMngr::col8_val.Trim()==L"ALL"||CPassedBYMngr::col8_val.Trim()==L"")&&(CPassedBYMngr::col9_val.Trim()==col_row_val[9].Trim() ||CPassedBYMngr::col9_val.Trim()==L"ALL"||CPassedBYMngr::col9_val.Trim()==L"") && (CPassedBYMngr::col10_val.Trim()==col_row_val[10].Trim() || CPassedBYMngr::col10_val.Trim()==L"ALL"||CPassedBYMngr::col10_val.Trim()==L"")&&(CPassedBYMngr::col11_val.Trim()==col_row_val[11].Trim() || CPassedBYMngr::col11_val.Trim()==L"ALL"||CPassedBYMngr::col11_val.Trim()==L"") && (CPassedBYMngr::col12_val.Trim()==col_row_val[12].Trim() || CPassedBYMngr::col12_val.Trim()==L"ALL"||CPassedBYMngr::col12_val.Trim()==L"")&& (CPassedBYMngr::col13_val.Trim()==col_row_val[13].Trim() || CPassedBYMngr::col13_val.Trim()==L"ALL"||CPassedBYMngr::col13_val.Trim()==L"") && (CPassedBYMngr::col14_val.Trim()==col_row_val[14].Trim() || CPassedBYMngr::col14_val.Trim()==L"ALL"||CPassedBYMngr::col14_val.Trim()==L""))
			{						
				CPassedBYMngr::m_st_Dealing_Grid_array.Add(&m_st_Netposition);						
			}

		}
		else
		{
			CPassedBYMngr::m_st_Dealing_Grid_array.Assign(CPassedBYMngr::m_st_Dealing_Array);	

		}

	}



	//Shorting Column
	int t_rows=m_st_Dealing_Grid_array.Total();
	st_Dealing first_st={};
	st_Dealing swap_st={};
	st_Dealing next_st={};


	int val_type=0;
	if (col_click==1||col_click==2)
	{
		val_type=0;
	}
	else
	{
		val_type=1;
	}
	for (int i=0;i<t_rows;i++)
	{
		first_st=m_st_Dealing_Grid_array[i];
		for (int j=i+1;j<t_rows;j++)
		{
			next_st=m_st_Dealing_Grid_array[j];
			if (a_d==0)
			{
				if (val_type==0)
				{
					if (col_click==1)
					{
						if (wcscmp(first_st.Section ,next_st.Section )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Section ,swap_st.Section );							
						}
					}
					if (col_click==2)
					{
						if (wcscmp(first_st.Login  ,next_st.Login  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Login  ,swap_st.Login );							
						}
					}
				}
				else
				{
					if (col_click==3)
					{															
						double d_val1=first_st.Order ;															
						double d_val2=next_st.Order;						
						if (d_val1>d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Order =swap_st.Order ;							
						}
					}
					//Order_In_Time
					if (col_click==4)
					{															
						if (wcscmp(first_st.Order_In_Time  ,next_st.Order_In_Time  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Order_In_Time  ,swap_st.Order_In_Time );							
						}
					}
					if (col_click==5)
					{															
						int d_val1=first_st.Deal ;															
						int d_val2=next_st.Deal;						
						if (d_val1>d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Deal =swap_st.Deal ;							
						}
					}
					if (col_click==6)
					{															
						if (wcscmp(first_st.Symbol  ,next_st.Symbol  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Symbol  ,swap_st.Symbol );							
						}
					}
					if (col_click==7)
					{															
						if (wcscmp(first_st.Type1  ,next_st.Type1  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Type1  ,swap_st.Type1 );							
						}
					}
					if (col_click==8)
					{															
						double d_val1=first_st.Volume  ;															
						double d_val2=next_st.Volume;
						if (d_val1>d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Volume =swap_st.Volume;							
						}
					}
					if (col_click==9)
					{															
						double d_val1=first_st.Price   ;															
						double d_val2=next_st.Price;
						if (d_val1>d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Price =swap_st.Price;					
						}
					}
					if (col_click==10)
					{															
						if (wcscmp(first_st.Comment  ,next_st.Comment  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Comment  ,swap_st.Comment );							
						}
					}
					if (col_click==11)
					{															
						if (wcscmp(first_st.Status  ,next_st.Status  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Status  ,swap_st.Status );							
						}
					}
					if (col_click==12)
					{															
						if (wcscmp(first_st.Type  ,next_st.Type  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Type  ,swap_st.Type );							
						}
					}
					if (col_click==13)
					{															
						if (wcscmp(first_st.SubType  ,next_st.SubType  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.SubType  ,swap_st.SubType );							
						}
					}
					if (col_click==14)
					{															
						if (wcscmp(first_st.PassedTime  ,next_st.PassedTime  )>0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.PassedTime  ,swap_st.PassedTime );							
						}
					}

				}
			}
			else
			{
				if (val_type==0)
				{
					if (col_click==1)
					{
						if (wcscmp(first_st.Section ,next_st.Section )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Section ,swap_st.Section );							
						}
					}
					if (col_click==2)
					{
						if (wcscmp(first_st.Login  ,next_st.Login  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Login  ,swap_st.Login );							
						}
					}
				}
				else
				{
					if (col_click==3)
					{															
						double d_val1=first_st.Order ;															
						double d_val2=next_st.Order;						
						if (d_val1<d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Order =swap_st.Order ;							
						}
					}

					if (col_click==4)
					{															
						if (wcscmp(first_st.Order_In_Time  ,next_st.Order_In_Time  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Order_In_Time  ,swap_st.Order_In_Time );							
						}
					}
					if (col_click==5)
					{															
						int d_val1=first_st.Deal ;															
						int d_val2=next_st.Deal;						
						if (d_val1<d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Deal =swap_st.Deal ;							
						}
					}
					if (col_click==6)
					{															
						if (wcscmp(first_st.Symbol  ,next_st.Symbol  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Symbol  ,swap_st.Symbol );							
						}
					}
					if (col_click==7)
					{															
						if (wcscmp(first_st.Type1  ,next_st.Type1  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Type1  ,swap_st.Type1 );							
						}
					}
					if (col_click==8)
					{															
						double d_val1=first_st.Volume  ;															
						double d_val2=next_st.Volume;
						if (d_val1<d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Volume =swap_st.Volume;							
						}
					}
					if (col_click==9)
					{															
						double d_val1=first_st.Price   ;															
						double d_val2=next_st.Price;
						if (d_val1<d_val2)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							first_st.Price =swap_st.Price;					
						}
					}
					if (col_click==10)
					{															
						if (wcscmp(first_st.Comment  ,next_st.Comment  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Comment  ,swap_st.Comment );							
						}
					}
					if (col_click==11)
					{															
						if (wcscmp(first_st.Status  ,next_st.Status)<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Comment  ,swap_st.Comment );							
						}
					}
					if (col_click==12)
					{															
						if (wcscmp(first_st.Type  ,next_st.Type  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.Type  ,swap_st.Type );							
						}
					}
					if (col_click==13)
					{															
						if (wcscmp(first_st.SubType  ,next_st.SubType  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.SubType  ,swap_st.SubType );							
						}
					}
					if (col_click==14)
					{															
						if (wcscmp(first_st.PassedTime  ,next_st.PassedTime  )<0)
						{
							m_st_Dealing_Grid_array.Shift(j,i-j);
							swap_st=m_st_Dealing_Grid_array[i];
							CMTStr::Copy(first_st.PassedTime  ,swap_st.PassedTime );							
						}
					}

				}

			}
		}
	}
	//End of Shorting Col



	int r_count=m_st_Dealing_Grid_array.Total();
	CPassedBYMngr::dealing_mutex.Unlock();
	int grid_total=GetNumberRows();

	if (CPassedBYMngr::insertFilterFlag==1)
	{
		r_count=r_count+1;
	}		
	if (grid_total!=r_count)
	{			
		if (r_count>0)
		{
			SetNumberRows(r_count);					
		}
	}
	else
	{			
		RedrawAll();			
	}					
}		


void CPassedBYMngr::OnGetCell(int col,long row,CUGCell *cell)
{
	//m_logfile_g.LogEvent(L"Start OnGetCell");
	st_Dealing mst_grid={};
	int rows_no=0;
	rows_no=row;		
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*cell);		
	if ( col >= 0 && row == -1 )
	{	
	}
	else if ( row >= 0 && col == -1 )
	{	
	}
	else if ( col >= 0 && row >= 0 )
	{
		if (CPassedBYMngr::insertFilterFlag==1)
		{
			rows_no=row-1;				
			if (row==0)
			{
				return;
			}
		}
		if (col==0)
		{				

			int r_val=0;
			if(CPassedBYMngr::insertFilterFlag==0)
			{
				r_val=row+1;
			}
			else
			{
				r_val=row;
			}
			cell->SetNumber(r_val);

		}
		else if (col==1)
		{				
			mst_grid=m_st_Dealing_Grid_array[rows_no];				
			CString tmp=mst_grid.Section ;				
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==2)
		{

			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.Login ;
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==3)
		{				
			mst_grid=m_st_Dealing_Grid_array[rows_no];
			double m_order=mst_grid.Order  ;

			CString tmp=L"" ;
			tmp.Format(L"%.0f",m_order);
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==4)
		{			
			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.Order_In_Time ;				
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==5)
		{				
			mst_grid=m_st_Dealing_Grid_array[rows_no];
			int m_deal=mst_grid.Deal ;

			CString tmp=L"" ;
			tmp.Format(L"%d",m_deal);
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==6)
		{				
			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.Symbol ;
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,mst_grid.Symbol)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==7)
		{	
			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.Type1;
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==8)
		{				
			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=L"";
			tmp.Format(L"%.2f",mst_grid.Volume);
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==9)
		{	

			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=L"";
			tmp.Format(L"%.4f",mst_grid.Price);											
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==10)
		{	

			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.Comment;										
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==11)
		{	

			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.Status;										
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==12)
		{	

			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.Type;

			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==13)
		{	

			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.SubType;										
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
		else if (col==14)
		{	

			mst_grid=m_st_Dealing_Grid_array[rows_no];
			CString tmp=mst_grid.PassedTime ;										
			CString str_get_value=cell->GetText();
			if (wcscmp(str_get_value,tmp)!=0)
			{
				cell->SetText(tmp);
			}
		}
	}

}


void CPassedBYMngr::OnMenuCommand(int col,long row,int section,int item)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(section);
	// init. local variables

	switch( item )
	{
	case 2001:
		{
			filter();
			break;
		}		  
	}
}
void CPassedBYMngr::filter()
{
	try
	{
		CMenu *pMnenu;
		pMnenu= GetPopupMenu();

		if ( CPassedBYMngr::insertFilterFlag==0)
		{
			addItemToCombobox();
			CPassedBYMngr::insertFilterFlag=1;
			InsertRow(0);
			for (int col_count=0;col_count<15;col_count++)
			{
				CUGCell cell;
				int row=0;
				int col=col_count;
				cell.ClearAll();
				cell.SetCellType(UGCT_DROPLIST);
				cell.SetCellTypeEx(UGCT_DROPLISTHIDEBUTTON);
				cell.SetParam(CELLTYPE_IS_EDITABLE);
				QuickSetRange(col,row,col,row,&cell);
				QuickSetLabelText(col,row,str[col_count]);
			}
			pMnenu->CheckMenuItem(2001,MF_CHECKED);

		}
		else
		{

			DeleteRow(0);
			CPassedBYMngr::insertFilterFlag=0;
			CPassedBYMngr::col0_val=L"";
			CPassedBYMngr::col1_val=L"";
			CPassedBYMngr::col2_val=L"";
			CPassedBYMngr::col3_val=L"";
			CPassedBYMngr::col4_val=L"";
			CPassedBYMngr::col5_val=L"";
			CPassedBYMngr::col6_val=L"";
			CPassedBYMngr::col7_val=L"";
			CPassedBYMngr::col8_val=L"";
			CPassedBYMngr::col9_val=L"";
			CPassedBYMngr::col10_val=L"";
			CPassedBYMngr::col11_val=L"";
			CPassedBYMngr::col12_val=L"";
			CPassedBYMngr::col13_val=L"";
			CPassedBYMngr::col14_val=L"";
			pMnenu->CheckMenuItem(2001,MF_UNCHECKED);
		}
		RedrawAll();
	}
	catch(_com_error & ce)
	{
		AfxMessageBox(ce.Description()+L"   filter");			
	} 
}



void CPassedBYMngr::addItemToCombobox()
{
	CStringArray arr;
	CStringArray arr1;
	CStringArray arr2;
	CStringArray arr3;	
	CStringArray arr4;
	CStringArray arr5;
	CStringArray arr6;	
	CStringArray arr7;	
	CStringArray arr8;	
	CStringArray arr9;
	CStringArray arr10;	
	CStringArray arr11;	
	CStringArray arr12;

	CStringArray arr13;
	CStringArray arr14;
	try
	{
		int rows=1;
		rows=m_st_Dealing_Array.Total();	
		CString str_val=L"";	
		for (int forcount=0;forcount<15;forcount++)
		{
			str[forcount]=L"ALL\n";		
		}
		for (int forcount=0;forcount<rows;forcount++)
		{
			st_Dealing m_st_Netposition={};
			m_st_Netposition=m_st_Dealing_Array[forcount];
			for (int clocount=0;clocount<15;clocount++)
			{		
				if (clocount==0)
				{

					str_val.Format(L"%d",m_st_Netposition.SerialNo);
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr,str_val)==false && str_val!=L"")
					{
						str[0]=str[0]+str_val+L"\n";										
						arr.Add(str_val);							
					}
				}

				if (clocount==1)
				{

					str_val=m_st_Netposition.Section;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr1,str_val)==false && str_val!=L"")
					{
						str[1]=str[1]+str_val+L"\n";										
						arr.Add(str_val);							
					}
				}
				if (clocount==2)
				{
					str_val=m_st_Netposition.Login ;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr2,str_val)==false  && str_val!=L"")
					{
						str[2]=str[2]+str_val+L"\n";										
						arr1.Add(str_val);
					}
				}


				if (clocount==3)
				{
					double  u_deal=m_st_Netposition.Order  ;
					str_val.Format(L"%.0f",u_deal );
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr3,str_val)==false  && str_val!=L"")
					{
						str[3]=str[3]+str_val+L"\n";										
						arr3.Add(str_val);
					}
				}


				if (clocount==4)
				{

					str_val=m_st_Netposition.Order_In_Time;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr4,str_val)==false  && str_val!=L"")
					{
						str[4]=str[4]+str_val+L"\n";										
						arr4.Add(str_val);
					}
				}

				if (clocount==5)
				{

					str_val.Format(L"%d",m_st_Netposition.Deal );
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr5,str_val)==false  && str_val!=L"")
					{
						str[5]=str[5]+str_val+L"\n";										
						arr5.Add(str_val);
					}
				}
				if (clocount==6)
				{
					str_val=m_st_Netposition.Symbol;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr6,str_val)==false  && str_val!=L"")
					{
						str[6]=str[6]+str_val+L"\n";										
						arr6.Add(str_val);
					}
				}

				if (clocount==7)
				{
					str_val=m_st_Netposition.Type1  ;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr7,str_val)==false  && str_val!=L"")
					{
						str[7]=str[7]+str_val+L"\n";										
						arr7.Add(str_val);
					}
				}
				if (clocount==8)
				{

					str_val.Format(L"%.2f",m_st_Netposition.Volume);
					if (CheckvalueInArray(arr8,str_val)==false  && str_val!=L"")
					{
						str[8]=str[8]+str_val+L"\n";										
						arr8.Add(str_val);
					}
				}
				if (clocount==9)
				{						
					str_val.Format(L"%.4f",m_st_Netposition.Price);
					if (CheckvalueInArray(arr9,str_val)==false  && str_val!=L"")
					{
						str[9]=str[9]+str_val+L"\n";										
						arr9.Add(str_val);
					}
				}	
				if (clocount==10)
				{	
					//str_val=m_st_Netposition.Comment;
					str_val.Format(L"%s",m_st_Netposition.Comment);

					str_val=str_val.Trim();
					if (CheckvalueInArray(arr10,str_val)==false  && str_val!=L"")
					{
						str[10]=str[10]+str_val+L"\n";										
						arr10.Add(str_val);
					}
				}	
				if (clocount==11)
				{						
					str_val=m_st_Netposition.Status;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr11,str_val)==false  && str_val!=L"")
					{
						str[11]=str[11]+str_val+L"\n";										
						arr11.Add(str_val);
					}
				}
				if (clocount==12)
				{						
					str_val=m_st_Netposition.Type;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr12,str_val)==false  && str_val!=L"")
					{
						str[12]=str[12]+str_val+L"\n";										
						arr12.Add(str_val);
					}
				}
				if (clocount==13)
				{						
					str_val=m_st_Netposition.SubType;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr13,str_val)==false  && str_val!=L"")
					{
						str[13]=str[13]+str_val+L"\n";										
						arr13.Add(str_val);
					}
				}
				if (clocount==14)
				{						
					str_val=m_st_Netposition.PassedTime ;
					str_val=str_val.Trim();
					if (CheckvalueInArray(arr14,str_val)==false  && str_val!=L"")
					{
						str[14]=str[14]+str_val+L"\n";										
						arr14.Add(str_val);
					}
				}
			}												
		}

	}
	catch(_com_error & ce)
	{
		AfxMessageBox(ce.Description()+L"addItemToCombobox");			
	} 

	CPassedBYMngr::dealing_mutex.Unlock();	
}

BOOLEAN  CPassedBYMngr::CheckvalueInArray(const CStringArray& arr,CString strval) 
{
	try
	{
		int arrcount=arr.GetCount();
		for (int forcount=0;forcount<arrcount;forcount++)
		{
			if (arr[forcount]==strval)
			{
				return true;
			}
		}
		return false  ;
	}
	catch(_com_error & ce)
	{
		AfxMessageBox(ce.Description()+L"CheckvalueInArray");			
	} 
}


UINT PassedByMnger(void *pParam);
UINT PassedByMnger(void *pParam)
{
	CPassedBYMngr* pThis= (CPassedBYMngr*)pParam;	
	CoInitialize(NULL);
	CDataSource connection;
	CSession session;
	CCommand<CAccessor<CPassedBYMngrTable> > artists1;	
	HRESULT hr;
	hr=connection.OpenFromInitializationString(L"Provider=SQLNCLI11.1;Password=ok@12345;Persist Security Info=False;User ID=sa;Initial Catalog=Tradedatabase;Data Source=64.251.7.161;Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=WINDOWS-LOJSHQK;Initial File Name=\"\";Use Encryption for Data=False;Tag with column collation when possible=False;MARS Connection=False;DataTypeCompatibility=0;Trust Server Certificate=False;Application Intent=READWRITE");
	if(SUCCEEDED(hr))
	{
		hr=session.Open(connection);
		while (CPassedBYMngr::Data_Update==1)
		{			
			Sleep(100);
			CString strCommand=L"";		
			//strCommand.Format(L"proc_Type1");    
			strCommand.Format(L"proc_Passed_By_Manager");    			
			_bstr_t bstrCommand="";
			bstrCommand=strCommand;
			char* strCommand_char=(char*)bstrCommand;
			if(SUCCEEDED(hr))
			{
				hr=artists1.Open(session,strCommand_char);							 
			}		
			CPassedBYMngr::dealing_mutex.Lock();
			if(SUCCEEDED(hr))
			{
				CPassedBYMngr::m_st_Dealing_Array.Clear();
				CPassedBYMngr::st_Dealing m_st_Dealing={};

				while (artists1.MoveNext() == S_OK)
				{																	  
					CMTStr::Copy(m_st_Dealing.Section ,artists1.m_Section );				 					
					CMTStr::Copy(m_st_Dealing.Login ,artists1.m_login );				 										
					m_st_Dealing.Order =artists1.m_Order;
					CMTStr::Copy(m_st_Dealing.Order_In_Time,artists1.m_Order_In_Time ) ;
					m_st_Dealing.Deal=artists1.m_Deal ;
					CMTStr::Copy(m_st_Dealing.Symbol ,artists1.m_Symbol );		
					CMTStr::Copy(m_st_Dealing.Type1 ,artists1.m_Type1 );		
					m_st_Dealing.Volume =artists1.m_Volume ;
					m_st_Dealing.Price =artists1.m_Price;
					CMTStr::Copy(m_st_Dealing.Comment,artists1.m_Comment);
					CMTStr::Copy(m_st_Dealing.Status,artists1.m_Status);
					CMTStr::Copy(m_st_Dealing.Type,artists1.m_Type);
					CMTStr::Copy(m_st_Dealing.SubType,artists1.m_SubType);
					CMTStr::Copy(m_st_Dealing.PassedTime ,artists1.m_PassedTime);
					CPassedBYMngr::m_st_Dealing_Array.Add(&m_st_Dealing);
				}
				artists1.Close();				    									 			 				 
			}
			CPassedBYMngr::dealing_mutex.Unlock();

			Sleep(1000);

		}
	}

	connection.Close();
	session.Close();
	return 0;
}



void CPassedBYMngr::OnDClicked(int col,long row,RECT *rect,POINT *point,BOOL processed)
{
	CUGCell cell;
	GetCell(col,row,&cell);
	int nCellTypeIndex = cell.GetCellType();
	int nParam = cell.GetParam();
	CString string;
	if(cell.GetLabelText() != NULL)
		string = cell.GetLabelText();

	if(processed){
		if(cell.GetCellType() == m_nSpinIndex)
			return ;
	}

	if(nParam == CELLTYPE_IS_EDITABLE || string == "CELLTYPE_IS_EDITABLE")
	{
		StartEdit();
	}


}
int CPassedBYMngr::OnDropList(long ID,int col,long row,long msg,long param)
{
	if (msg==103)
	{
		if(CPassedBYMngr::insertFilterFlag==1 && row==0)
		{
			CString  strval=L"";
			CUGCell cell;
			GetCell(col,row,&cell);
			strval=cell.GetText();	
		}

		if(CPassedBYMngr::insertFilterFlag==1 && row==0 )
		{

			CString  strval=L"";
			CUGCell cell;
			GetCell(col,row,&cell);
			strval=cell.GetText();	
			if(col==0)
			{
				if (strval!=L"")
				{
					col0_val=strval;					
				}
				else
				{
					col0_val=L"ALL";					
				}
			}


			if(col==1)
			{
				if (strval!=L"")
				{
					col1_val=strval;					
				}
				else
				{
					col1_val=L"ALL";					
				}
			}

			if(col==2)
			{
				if (strval!=L"")
				{
					col2_val=strval;					
				}
				else
				{
					col2_val=L"ALL";					
				}
			}

			if(col==3)
			{
				if (strval!=L"")
				{
					col3_val=strval;					
				}
				else
				{
					col3_val=L"ALL";					
				}
			}

			if(col==4)
			{
				if (strval!=L"")
				{
					col4_val=strval;					
				}
				else
				{
					col4_val=L"ALL";					
				}
			}

			if(col==5)
			{
				if (strval!=L"")
				{
					col5_val=strval;					
				}
				else
				{
					col5_val=L"ALL";					
				}
			}

			if(col==6)
			{
				if (strval!=L"")
				{
					col6_val=strval;					
				}
				else
				{
					col6_val=L"ALL";					
				}
			}

			if(col==7)
			{
				if (strval!=L"")
				{
					col7_val=strval;					
				}
				else
				{
					col7_val=L"ALL";					
				}
			}

			if(col==8)
			{
				if (strval!=L"")
				{
					col8_val=strval;					
				}
				else
				{
					col8_val=L"ALL";					
				}
			}

			if(col==9)
			{
				if (strval!=L"")
				{
					col9_val=strval;					
				}
				else
				{
					col9_val=L"ALL";					
				}
			}

			if(col==10)
			{
				if (strval!=L"")
				{
					col10_val=strval;					
				}
				else
				{
					col10_val=L"ALL";					
				}
			}

			if(col==11)
			{
				if (strval!=L"")
				{
					col11_val=strval;					
				}
				else
				{
					col11_val=L"ALL";					
				}
			}

			if(col==12)
			{
				if (strval!=L"")
				{
					col12_val=strval;					
				}
				else
				{
					col12_val=L"ALL";					
				}
			}

			if(col==13)
			{
				if (strval!=L"")
				{
					col13_val=strval;					
				}
				else
				{
					col13_val=L"ALL";					
				}
			}

			if(col==14)
			{
				if (strval!=L"")
				{
					col14_val=strval;					
				}
				else
				{
					col14_val=L"ALL";					
				}
			}
		}
	}

	return true;

}

int CPassedBYMngr::OnEditFinish(int col, long row,CWnd *edit,LPCTSTR string,BOOL cancelFlag)
{

	if(CPassedBYMngr::insertFilterFlag==1 && row==0)
	{
		CString  strval=L"";
		CUGCell cell;
		GetCell(col,row,&cell);
		cell.GetText();
		strval=string;	
	}

	if(CPassedBYMngr::insertFilterFlag==1 && row==0 )
	{

		CString  strval=L"";
		CUGCell cell;
		QuickSetText(col,row,string);
		GetCell(col,row,&cell);
		strval=cell.GetText();	
		if(col==0)
		{
			if (strval!=L"")
			{
				col0_val=strval;					
			}
			else
			{
				col0_val=L"ALL";					
			}
		}


		if(col==1)
		{
			if (strval!=L"")
			{
				col1_val=strval;					
			}
			else
			{
				col1_val=L"ALL";					
			}
		}

		if(col==2)
		{
			if (strval!=L"")
			{
				col2_val=strval;					
			}
			else
			{
				col2_val=L"ALL";					
			}
		}

		if(col==3)
		{
			if (strval!=L"")
			{
				col3_val=strval;					
			}
			else
			{
				col3_val=L"ALL";					
			}
		}

		if(col==4)
		{
			if (strval!=L"")
			{
				col4_val=strval;					
			}
			else
			{
				col4_val=L"ALL";					
			}
		}

		if(col==5)
		{
			if (strval!=L"")
			{
				col5_val=strval;					
			}
			else
			{
				col5_val=L"ALL";					
			}
		}

		if(col==6)
		{
			if (strval!=L"")
			{
				col6_val=strval;					
			}
			else
			{
				col6_val=L"ALL";					
			}
		}

		if(col==7)
		{
			if (strval!=L"")
			{
				col7_val=strval;					
			}
			else
			{
				col7_val=L"ALL";					
			}
		}

		if(col==8)
		{
			if (strval!=L"")
			{
				col8_val=strval;					
			}
			else
			{
				col8_val=L"ALL";					
			}
		}

		if(col==9)
		{
			if (strval!=L"")
			{
				col9_val=strval;					
			}
			else
			{
				col9_val=L"ALL";					
			}
		}

		if(col==10)
		{
			if (strval!=L"")
			{
				col10_val=strval;					
			}
			else
			{
				col10_val=L"ALL";					
			}
		}

		if(col==11)
		{
			if (strval!=L"")
			{
				col11_val=strval;					
			}
			else
			{
				col11_val=L"ALL";					
			}
		}

		if(col==12)
		{
			if (strval!=L"")
			{
				col12_val=strval;					
			}
			else
			{
				col12_val=L"ALL";					
			}
		}

		if(col==13)
		{
			if (strval!=L"")
			{
				col13_val=strval;					
			}
			else
			{
				col13_val=L"ALL";					
			}
		}

		if(col==14)
		{
			if (strval!=L"")
			{
				col14_val=strval;					
			}
			else
			{
				col14_val=L"ALL";					
			}
		}
	}

	return true;
}

void CPassedBYMngr::OnTH_LClicked(int col,long row,int updn,RECT *rect,POINT *point,BOOL processed)
{		
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(rect);
	UNREFERENCED_PARAMETER(point);
	UNREFERENCED_PARAMETER(processed);

	col_click=col;
	if( updn == 0)
		return;
	if (col_click!=col)
	{
		a_d=0;
	}
	else
	{
		if (a_d==0)
		{
			a_d=1;
		}
		else
		{
			a_d=0;
		}
	}


	QuickSetCellType( m_iSortCol, -1, 0 );

	if( col == m_iSortCol )
	{
		if( m_bSortedAscending )
			m_bSortedAscending = FALSE;
		else
			m_bSortedAscending = TRUE;
	}
	else
	{
		m_iSortCol = col;
		m_bSortedAscending = TRUE;
	}

	if( m_bSortedAscending )
	{
		SortBy( col, UG_SORT_ASCENDING );
		QuickSetCellType( m_iSortCol, -1, m_iArrowIndex );
		QuickSetCellTypeEx( m_iSortCol, -1, UGCT_SORTARROWDOWN );

		//		((CFindAndSortDemo*)GetParent())->Trace( _T( "Sorted column %d ascending" ), iCol );
	}
	else
	{
		SortBy( col, UG_SORT_DESCENDING );
		QuickSetCellType( m_iSortCol, -1, m_iArrowIndex );
		QuickSetCellTypeEx( m_iSortCol, -1, UGCT_SORTARROWUP );
		//		Trace( _T( "Sorted column %d descending" ), iCol );
	}			 				
	RedrawAll();
}

void CPassedBYMngr::thread_destoy()
{
	try 
	{	
		Data_Update=0;
		DWORD exit_code= NULL;

		if (m_pThreads != NULL)
		{
			if(WaitForSingleObject(m_pThreads->m_hThread,INFINITE) == WAIT_OBJECT_0) 
			{
				GetExitCodeThread(m_pThreads->m_hThread, &exit_code);
				if(exit_code == STILL_ACTIVE)
				{
					::TerminateThread(m_pThreads->m_hThread, 0);
					CloseHandle(m_pThreads->m_hThread);
				}
				//m_pThreads->m_hThread = NULL;
				m_pThreads = NULL;
			}
		}
	}
	catch(_com_error & ce)
	{
		AfxMessageBox(ce.Description()+L"Thread UnInitiliaze");			
	}
}