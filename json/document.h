...
//#warning for keeping rhs intact
            //		this->~GenericValue();
...
//            o.members[o.size].value.RawAssign(value);
            ...
            if (key == "__type") {
                ...
                for (int i = o.size; i >= 0; i--) {
                    o.members[i].name.RawAssign(o.members[i-1].name);
                    o.members[i].value.RawAssign(o.members[i-1].value);
                }
                o.members[0u].name.RawAssign(name);
                o.members[0u].value.RawAssign(value);
            } else {
                o.members[o.size].name.RawAssign(name);
                o.members[o.size].value.RawAssign(value);
            }
        ...
        GenericValue& PushBack(GenericValue& value, int index, Allocator& allocator) {
            RAPIDJSON_ASSERT(IsArray());
            RAPIDJSON_ASSERT(index < (data_.a.size + 1));
            RAPIDJSON_ASSERT(index >= 0);
            if (data_.a.size >= data_.a.capacity)
                Reserve(data_.a.capacity == 0 ? kDefaultArrayCapacity : data_.a.capacity * 2, allocator);


            for (rapidjson::SizeType i = data_.a.size; i > index ; i--) {
                data_.a.elements[rapidjson::SizeType(i)].RawAssign(data_.a.elements[rapidjson::SizeType(i - 1)]);
            }


            data_.a.elements[rapidjson::SizeType(index)].RawAssign(value);
            data_.a.size++;
